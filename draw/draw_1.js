...
function Ellipse(color,strokeWidth,shape,x,y,a,b)
{
	Shape.call(this,color,strokeWidth,shape);
	this.x = x;
	this.y = y;  //圆心
...
function MyCanvas()
{
	...
	this.openCanvas = function(openLi){
		...
		str +='<li onclick="localStorage.clear();" style="color:red">清空</li></ul>';
	...
	_this.drawCurve = function(oShape){
	    ...
	    context.lineWidth = oShape.strokeWidth;
		...
		if(isMouseDown && action == 1){
			...
			switch(shape){
				...
				case 15 :
					...
					var ellipse = new Ellipse(color,strokeWidth,shape,eX,eY,a,b);
		...
		if(action == 1 && isMouseDown)
		{
			...
			switch(shape){
				...
				case 14 :
					...
					var rectAngle = new RectAngle(color,strokeWidth,shape,orignalX, orignalY,w,h );
					data.push(rectAngle);
					break;
				case 15 :
					var eX = orignalX+(lastX-orignalX)/2;
